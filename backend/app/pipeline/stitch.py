...
def _wrap_blur(img, sigma):
    """Gaussian blur that wraps horizontally (the equirect's 360° seam) so the
    left/right join blends like every other seam; vertical uses reflect."""
    pad = max(1, int(sigma * 3))
    padded = np.concatenate([img[:, -pad:], img, img[:, :pad]], axis=1)
    b = cv2.GaussianBlur(padded, (0, 0), sigmaX=sigma, sigmaY=sigma, borderType=cv2.BORDER_REFLECT)
    return b[:, pad:pad + img.shape[1]]
...
def _stitch_oriented(image_paths, orientations, width=TARGET_WIDTH):
    ...
    #  (1) winner-take-all → crisp DETAIL: each pixel from the single most-centred
    #      photo, so overlaps are never averaged → no ghost-doubling.
    #  (2) a feathered average → smooth BRIGHTNESS across overlaps.
    # We then multi-band blend: keep the HIGH frequencies of (1) and the LOW
    # frequencies of (2). Detail stays sharp; the exposure jumps at the seams are
    # low-frequency, so they melt into a smooth gradient and the seams disappear.
    sharp = np.full((H, W, 3), 16.0, np.float32)
    ...
    for path, ori in zip(image_paths, orientations):
        ...
        center = np.clip(1 - np.maximum(np.abs(nx), np.abs(ny)), 0, 1)
        # (1) winner-take-all on centeredness
        weight = np.where(inside, center, -1.0)
        take = weight > best
        sharp[take] = warped[take]
        best[take] = weight[take]
        # (2) feathered average (wide overlap) for smooth brightness
        wf = (np.where(inside, center, 0.0) ** 2).astype(np.float32)
        acc += warped * wf[..., None]
        wsum += wf
        placed += 1
    ...
    covered = best >= 0
    smooth = np.where(wsum[..., None] > 1e-6, acc / np.maximum(wsum[..., None], 1e-6), sharp)
    sigma = W / 120.0                                   # low-band width (~seam scale)
    blended = sharp + (_wrap_blur(smooth, sigma) - _wrap_blur(sharp, sigma))
    out = np.where(covered[..., None], np.clip(blended, 0, 255), 16.0)
